...
export function createTextWindow(parent: HTMLElement = document.body): TextWindowHandle {
  ...
  el.innerHTML = `<div class="card"><div class="head"><span></span><button type="button" aria-label="Close">✕</button></div><div class="content"></div><div class="writer"></div></div>`;
  ...
  return {
    ...
    show: (next) => {
      ...
      content.textContent = next.text || '(blank)';
